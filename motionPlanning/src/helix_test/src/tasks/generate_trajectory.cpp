...
void DemoApplication::generateTrajectory(DescartesTrajectory& traj)
{
  ...
  if(readPoseFile(config_.filename, poses, config_.offset))
  {
  ...
  for(unsigned int i = 0; i < poses.size(); i++)
  {
    ...
    descartes_core::TrajectoryPtPtr pt = descartes_core::TrajectoryPtPtr(new descartes_trajectory::CartTrajectoryPt(pose));
