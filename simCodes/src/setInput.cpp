...
setInput::setInput()
{
  ...
  // AddOption("threshold", "Threshold for judging buckling", threshold);
	// AddOption("thresholdr", "Normalized threshold for judging buckling", threshold);
  AddOption("v_constant", "Const velocity", v_constant);

	AddOption("c1", "slope of torsion over kap", c1);
	AddOption("c2","slope of theta over kap", c2);

	AddOption("kapB","boundary of kap", kapB);
	AddOption("experiment","record for the experiment data", experiment);
